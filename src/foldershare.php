...
This command is similar to the 'ls' command on Linux, BSD, and macOS.",
  ...
  //
  'lls' => [
    'synopsis' => 'lls [OPTIONS] LOCAL_PATH',
    'brief' => 'List files and folders.',
    'description' =>
...
supported.",
  ...
  //
  //Saves the remote path
  //
  'cd' => [
    'synopsis' => 'cd [OPTIONS] REMOTE_PATH',
    'brief' => 'Saves the remote path.',
    'description' =>
"Saves the remote path.
...
Both forms default to sorting items by name:
  ...
  //
  'lcd' => [
    'synopsis' => 'lcd [OPTIONS] LOCAL_PATH',
    'brief' => 'Saves the local path.',
    'description' =>
"Saves the local path.",
    'flags' => [
      '--help' => [
...
"Prints the remote path.",
  ...
  'whoami' => [
    'synopsis' => 'whoami',
    'brief' => 'name of logged in user',
    'description' =>
"Displays name of user logged in as",
    'flags' => [
      '--help' => [
        'brief' => 'Show this help information.',
      ],
...
This command is similar to the 'mv' command on Linux, BSD, and macOS.",
  ...
  'clear' => [
    'synopsis' => 'clear',
    'brief' => 'clears all current lines.',
    'description' =>
"clears the screen.",
    'flags' => [
      '--help' => [
...
The user must have permission to modify the remote files and folders.",
  ...
  'find' => [
    'synopsis' => 'find [OPTIONS] REMOTE_PATH',
    'brief' => 'Search remote filesystem.',
    'description' => <<<'EOS'
Searches for files in the foldershare filesystem
EOS
    ,
    'flags' => [
      '--help' => [
      ...
      '--name' => [
        'brief' => 'Specify a string to be searched for',
      ],
      '-R' => [
 ...
 */
function parseCommandFlags(string $appName, string $commandName, array $args, $directory) {
  //
  // Split args
  // ----------
  ...
  $paths = fixPaths($commandName, $paths, $directory);
  //
  // Expand flags
  // ------------
...
function fixPaths($commandName, $paths, $directory) {
  $localPaths = $paths;
  $cdPath = $directory[0];
  $lcdPath = $directory[1];
  ...
  switch($commandName) {
    ...
    case 'mv':
    case 'cp':
      for($i = 0; $i<count($localPaths); $i++) {
        $localPaths[$i] = organizeDirectory($localPaths[$i], $cdPath);
      }
      break;
    ...
    case 'lcd':
      for($i = 0; $i<count($localPaths); $i++) {
        $localPaths[$i] = organizeDirectory($localPaths[$i], $lcdPath);
      }
      break;

    case 'update':
      for($i = 2; $i<count($localPaths); $i++) {
        $localPaths[$i] = organizeDirectory($localPaths[$i], $cdPath);
      }
      break;

    case 'put':
      $localPaths[0] = organizeDirectory($localPaths[0], $lcdPath);
      $localPaths[1] = organizeDirectory($localPaths[1], $cdPath);
      break;

    case 'get':
      $localPaths[0] = organizeDirectory($localPaths[1], $cdPath);
      if(count($paths)>1) {
        $localPaths[1] = organizeDirectory($localPaths[0], $lcdPath);
      }
      break;
...
function organizeDirectory($newPath, $cdPath) {

  $retPath = '';
  if(strpos($newPath, '..') !== false) {
    $cdPath = mb_substr($cdPath, 0, strrpos($cdPath, '/'));
    $retPath = $cdPath;
  }
  else if($newPath == '/') {
    $retPath = '/';
  }
  else if(mb_substr($newPath, 0, 1) != "/"){
    $retPath = $cdPath.'/'.$newPath;
  }
  else {
    $retPath = $newPath;
  }

  $retPath = clearBackslashes($retPath);
...
function clearBackslashes($input) {
  while(mb_substr($input, -1) === '/') {
    $input = mb_substr($input, 0, mb_strlen($input)-1);
  }
  return $input;
}
 ...
 */
function runLls(FolderShareConnect $server, array $options){

  $localPath = "";
  $localPath = $options['paths'][0];

  $files = scandir($localPath);
  $files = array_diff(scandir($localPath), array('.', '..'));
...
/**
 * Jumps into and saves the remote path
 */
function runCd(FolderShareConnect $server, array $options) {
  if (count($options['paths']) === 1) {
    $remotePath = $options['paths'][0];
  }
  else if (count($options['paths']) === 0){
    echo "remote path missing"."\n"."\n";
    return;
  }
  else {
    echo "too many paths"."\n"."\n";
  ...
  try {
    ...
    $commandName = reset($options['command']);
    print("$commandName: " . $e->getMessage() . "\n");
    return $options['directory'][0];
  }

 $remotePath = clearBackslashes($remotePath);
  return $remotePath;
}

/**
 * Jumps into and saves the local path
 */
function runLcd(FolderShareConnect $server, array $options){
  if (count($options['paths']) === 1) {
    $localPath = $options['paths'][0];
  }
  else if (count($options['paths']) === 0){
  ...
  if(is_dir($localPath)) {
    ...
    echo "cd: \"".$localPath."\" could not be found.";
    echo "Please check that the path is correct.";
    return $options['directory'][1];
  }
}
function whoami(FolderShare $server, array $options) {
  print($options['username']);
}
/**
 * Prints the remote path
 */
function runPwd(FolderShareConnect $server, array $options){
  echo $options['directory'][0]."\n";
}

/**
 * Prints the local path
 */
function runLpwd(FolderShareConnect $server, array $options){
  echo $options['directory'][1]."\n";
}
...
function runCp(FolderShareConnect $server, array $options) {
  ...
  foreach ($remotePaths as $remotePath) {
    ...
    //Use the current cdPath to update the remotePath
    try {
      // There is normally no response.
 ...
 *   that same form.
 */
function runUpdate1(FolderShareConnect $server, array $options){
  print_r($options);
}
...
function runPut(FolderShareConnect $server, array $options){
  ...
  $options['paths'][0] = $localPath;
  $options['paths'][1] = $remotePath;

  //if the first input is a directory, calls the recursive loop method
  //uploads a folder and its contents
  if(is_dir($options['paths'][0])) {
    loop($options['paths'][0].'/',
    $options['paths'][1].'/'.basename($options['paths'][0], ""),
    $server,
    $options);
  }

  //if the first input is a file, calls the runPutFile method
  //uploads a file
  else{
    runPutFile($server, $options);
  }
...
function loop($dir, $newPath, FolderShareConnect $server, array $options){
  //calls the putDir method to make a new directory
  putDir($newPath, $server, $options);

  //creates an array of all items in the directory
  $ffs = scandir($dir);
  ...
  foreach($ffs as $ff){
    ...
    if(is_dir($dir.$ff)) {
      //if the item is a directory, calls the loop method again with that item
      loop($dir.$ff.'/', $newPath.'/'.$ff, $server, $options);
    }
    else if(strcmp($ff, ".DS_Store")!=0) {
      //if the item is a file, uploads that item
      putFile($dir.$ff, $newPath, $server, $options);
    }
  }
}


//uses mkDir to make a directory for the "loop" method
function putDir($path, FolderShareConnect $server, array $options) {
  echo "mkdir ".$path."\r\n";

  //updates the $options array to match runMkdir's inputs
  //the error is that format should be enclosed in quotes
  $inputArr = $options;
  $inputArr['format'] = 'keyvalue';
  $inputArr['command'] = array('mkdir', $path);
  $inputArr['paths'] = array($path);
  runMkdir($server, $inputArr);
}


//uses putFile to make a directory for the "loop" method
function putFile($path1, $path2, FolderShareConnect $server, array $options) {
  echo "putFile ".$path1." ".$path2."\r\n";

  //updates the $options array to match runPutFile's inputs
  $inputArr = $options;
  $inputArr['format'] = '';
  $inputArr['command'] = array('put', "$path1", "$path2");
  $inputArr['paths'] = array($path1, $path2);
  runPutFile($server, $inputArr);
}


function runPutFile(FolderShareConnect $server, array $options) {
  //
  // Parse options
  // -------------
  // Synonyms have already been mapped to primary flag names.
...
//Tab Complete Script
function tab($input1, $index){

  $line1 = "";
  global $options;
  $p = $options['directory'][0];
  $input = $input1;

  /*if($input[mb_strlen($input)-1] === '/' And mb_strlen($input) > 1){
    $input = mb_substr(0, mb_strlen($input)-1);
  }*/
  ...
  if($lastSlashPosition === (strlen($line1)-1) And $lastSlashPosition !== 0){
  ...
  $sup = mb_substr($parent, strlen($p)+1).'/';
  ...
  try {
    switch ($parent) {
      ...
      default:
        $server->setReturnFormat('keyvalue');
        $response = $server->getDescendants($parent);
        break;
    }
  }
  catch (\Exception $e){
  ...
  $formatFlags = [];
  if($response !== NULL){
    $response = FolderShareFormat::formatAsLinuxLs($response, $formatFlags);
  }

  $length = strlen($response);
  $parts = preg_split('/ {2}/', $response);

  if(strpos($parts[count($parts)-1],' ') === 0 Or $parts[count($parts)-1] === ''){
    if([count($parts)>1]){
  ...
  foreach ($parts as $part){
    while($part[count($part)-1] === ' ' Or $part[count($part)-1] === "\n"){
      $part = ltrim($part);
      $part = rtrim ($part);
    }
...
readline_completion_function("tab");
while (TRUE) {
  try {
    //
    // Prompt
    ...
    if ($options['directory'][0] == "")
      ...
      $prompt = "$appName {$options['directory'][0]}>";
    if (PHP_OS === 'WINNT') {
