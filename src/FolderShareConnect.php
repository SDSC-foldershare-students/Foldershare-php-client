  ...
  protected function verifyServer() {
    ...
    if ($httpCode !== 200) {
      ...
      if (isset($content['message']) === TRUE) {
        ...
        if ($httpCode === 404) {
          $message = <<<'EOS'
The host has declined the connection.
This can occur if the host does not have the FolderShare module enabled, or if web services for the module have not been enabled. Please contact the host's administrator.
EOS;
        }
        else {
...
The user name and/or password may be incorrect. Please check them for typos. If they appear correct, it is also possible that the host's web services are not configured to support the type of authentication used by this software. Please contact the host's administrator.
EOS
          );
      }
      else {
    ...
    try {
      ...
      if ($httpCode !== 403) {
        ...
        throw new \RuntimeException( <<<'EOS'
The login to the host failed.
The host is not responding properly to communications requests. This can occur if the host's web services are not enabled. Please contact the host's administrator.
EOS
          );
      }
...
web services. Please contact the host's administrator.");
  ...
  private function loginForCookie() {
    ...
    // - The return syntax.
    //http://drupal-8-5-6.dd:8083/user/login?_format=json
    $this->hostName = clearBackslashes($this->hostName);
